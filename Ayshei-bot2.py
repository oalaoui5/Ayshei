...
# Get user query through speech recognition
  # ... rest of your llm_function logic using the captured query ...
def llm_function():
...
def recognize_speech():
  """
  This function uses the SpeechRecognition library to capture user audio,
  convert it to text, and return the recognized text.
  """
...
def speak(text):
  """
  This function uses the gTTS library to convert text to speech and plays it.
  """
  tts = gTTS(text=text, lang='en')
  tts.save("output.mp3")
  os.system("mpg321 output.mp3")  # Play the generated audio file
  os.remove("output.mp3")  # Delete the temporary audio file
  ...
  with st.chat_message("assistant"):
    st.markdown(response_text)
    speak(response_text)  # Speak the assistant's response
